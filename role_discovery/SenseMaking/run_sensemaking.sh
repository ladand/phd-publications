...
${MATLAB} -r "javaaddpath('.'); V = dlmread('input/netsci-V.csv',',',1,1); A = spconvert(dlmread('output/test-A.csv')); [M, E, N, Q, E_names, Q_names, err] = MakeSense(A,V,'output/test-M.csv'); WriteRoleProperties(M, E, E_names, 'output/test-E-analysis.csv'); WriteRoleProperties(N, Q, Q_names, 'output/test-Q-analysis.csv'); WriteRoleProperties([M N], [E Q], [E_names Q_names], 'output/test-EQ-analysis.csv'); quit;"
